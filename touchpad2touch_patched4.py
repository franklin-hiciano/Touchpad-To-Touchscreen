...
class IndicatorOverlay(QtWidgets.QWidget):
    def __init__(self, size_px=18, fade_ms=120, grid_n=0, show_action_dot=False):
        ...
        # Sector (pie-wedge) overlay (centered on pointer prediction)
        self.sector = {
            "visible": False,
            "ax": 0.0, "ay": 0.0,      # apex on thumb–pinky line (in px)
            "R": 0.0,                  # outer radius (px)
            "deg_center": 0.0,         # centerline angle in Qt degrees (0°=east, CCW+)
            "deg_spread": 0.0          # total spread in degrees
        }
    ...
    def paintEvent(self, e):
        ...
        # Sector wedge (amethyst outline, translucent fill) anchored on TP-normal through apex, centered on pointer prediction
        S = getattr(self, "sector", None)
        if S and S["visible"] and S["R"] > 2.0:
            p.save()
            pen = QtGui.QPen(QtGui.QColor(155, 89, 182, 230))  # amethyst
            pen.setWidth(2)
            p.setPen(pen)
            p.setBrush(QtGui.QBrush(QtGui.QColor(155, 89, 182, 60)))
            ax, ay, R = S["ax"], S["ay"], S["R"]
            start_deg = S["deg_center"] - 0.5 * S["deg_spread"]
            span_deg  = S["deg_spread"]
            rect = QtCore.QRectF(ax - R, ay - R, 2*R, 2*R)
            p.drawPie(rect, int(start_deg * 16), int(span_deg * 16))
            p.restore()
    ...
    def set_sector(self, ax, ay, R, deg_center, deg_spread, visible=True):
        self.sector.update(
            ax=float(ax), ay=float(ay), R=float(R),
            deg_center=float(deg_center), deg_spread=float(deg_spread),
        ...
        if t == "ref_multi":
            ...
            if obj.get("clear"):
                if args.indicator:
                    ...
                    overlay.set_sector(0, 0, 0, 0, 0, False)
            else:
                import math
                ...
                # --- Sector centered on pointer prediction ---
                # Apex: projection of M onto T->P line (thumb–pinky baseline), centerline: normal to TP facing the predicted point
                # Radius: 2/3 rule => R = 1.5 * |A -> predicted|
                
                try:
                    vx_tp, vy_tp = (Px - Tx), (Py - Ty)
                    vv = vx_tp*vx_tp + vy_tp*vy_tp
                    if vv > 1e-6:
                        # Project predicted point onto TP to get apex A
                        wpx, wpy = (mx_pt - Tx), (my_pt - Ty)
                        tproj = (wpx*vx_tp + wpy*vy_tp) / vv
                        Ax = Tx + tproj * vx_tp
                        Ay = Ty + tproj * vy_tp

                        # Centerline from apex to predicted
                        dx, dy = (mx_pt - Ax), (my_pt - Ay)

                        # Radius so that predicted lies at 2/3 along centerline
                        dA_pred = math.hypot(dx, dy)
                        R_sec = max(float(getattr(args, "sector_min_radius_px", 60.0)), 1.5 * dA_pred)

                        # Qt angle (0°=east, CCW+; y down)
                        deg_center = math.degrees(math.atan2(-dy, dx))
                        deg_spread = float(getattr(args, "sector_angle_deg", 40.0))

                        if args.indicator:
                            overlay.set_sector(Ax, Ay, R_sec, deg_center, deg_spread, True)
                    else:
                        if args.indicator:
                            overlay.set_sector(0, 0, 0, 0, 0, False)
...
def main():
    ...
    # sector overlay tuning
    ap.add_argument("--sector-angle-deg", type=float, default=40.0,
                    help="Total central angle of the allowed pointer sector (degrees).")
    ap.add_argument("--sector-min-radius-px", type=float, default=60.0,
                    help="Minimum outer radius for the sector (pixels).")
